...
public class CalcUI extends javax.swing.JFrame {
   ...
   //UnitsChooser choozer = new UnitsChooser();
...
ShortMenu.setSelected(true);
FullMenu.setSelected(false);
    ...
    private void initComponents() {
        ...
        FullMenu = new javax.swing.JRadioButtonMenuItem();
        ShortMenu = new javax.swing.JRadioButtonMenuItem();
        ...
        UnitChooseCBx.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Длина", "Площадь", "Объем", "Плотность", "Температура", "Давление", "Скорость" }));
        ...
        FullMenu.setSelected(true);
        FullMenu.setText("Полный список");
        ...
        ShortMenu.setSelected(true);
        ShortMenu.setText("Краткий");
        ShortMenu.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ShortMenuActionPerformed(evt);
            }
        });
        jMenu3.add(ShortMenu);
 ...
 choose(UnitChooseCBx.getSelectedIndex(),FromCmBox.getSelectedIndex(),ToCmBx.getSelectedIndex(),LangMenuItem_Russ_.isSelected(),ShortMenu.isSelected());
...
ToTxtField.setText(converter.myFormatter(converter.convert(
    FromCmBox.getSelectedIndex(),ToCmBx.getSelectedIndex(),FromTxtField.getText(),(String)UnitChooseCBx.getSelectedItem(),ShortMenu.isSelected()),
...
ToTxtField.setText(converter.myFormatter(converter.convert(
    ...
    int tmpUnits = UnitChooseCBx.getSelectedIndex();
    int tmpUnitsValuesUp = FromCmBox.getSelectedIndex();
    int tmpUnitsValuesBottom = ToCmBx.getSelectedIndex();
    UnitChooseCBx.setModel(new javax.swing.DefaultComboBoxModel((LangMenuItem_Russ_.isSelected()) ? 
    ...
    choose(UnitChooseCBx.getSelectedIndex(),FromCmBox.getSelectedIndex(),ToCmBx.getSelectedIndex(),LangMenuItem_Russ_.isSelected(),ShortMenu.isSelected());
      UnitChooseCBx.setSelectedIndex(tmpUnits);
      FromCmBox.setSelectedIndex(tmpUnitsValuesUp);
      ToCmBx.setSelectedIndex(tmpUnitsValuesBottom);
    ...
    int tmp = UnitChooseCBx.getSelectedIndex();
    int tmpUnitsValuesUp = FromCmBox.getSelectedIndex();
    int tmpUnitsValuesBottom = ToCmBx.getSelectedIndex();
    UnitChooseCBx.setModel(new javax.swing.DefaultComboBoxModel((LangMenuItem_Russ_.isSelected()) ? 
       (new String [] {"Длина","Площадь","Объем","Плотность","Температура","Давление","Скорость"}):
       (new String [] {"Lenght","Area","Volume","Density","Temperature","Pressure","Velocity"})));
    choose(UnitChooseCBx.getSelectedIndex(),FromCmBox.getSelectedIndex(),ToCmBx.getSelectedIndex(),LangMenuItem_Russ_.isSelected(),ShortMenu.isSelected());
     UnitChooseCBx.setSelectedIndex(tmp);
     FromCmBox.setSelectedIndex(tmpUnitsValuesUp);
     ToCmBx.setSelectedIndex(tmpUnitsValuesBottom);
    ...
    private void FullMenuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_FullMenuActionPerformed
      if (ShortMenu.isSelected()) { ShortMenu.setSelected(false);
        choose(UnitChooseCBx.getSelectedIndex(), FromCmBox.getSelectedIndex(), ToCmBx.getSelectedIndex(), LangMenuItem_Russ_.isSelected(), ShortMenu.isSelected());}
    }//GEN-LAST:event_FullMenuActionPerformed

    private void ShortMenuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ShortMenuActionPerformed
       if (FullMenu.isSelected()) { FullMenu.setSelected(false);
        choose(UnitChooseCBx.getSelectedIndex(), FromCmBox.getSelectedIndex(), ToCmBx.getSelectedIndex(), LangMenuItem_Russ_.isSelected(), ShortMenu.isSelected());}
    }//GEN-LAST:event_ShortMenuActionPerformed
...
ToTxtField.setText(converter.myFormatter(converter.convert(
        ...
        switch (UnitChooseCBxIndex) {
            case 0://dlina
                if (LangMenuItem) {//Russian
                    FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "ангстрем","нанометр","микрометр","мм","cм","дц","м","чейн","км","дюйм","фут","ярд","мил(mil)","морская миля","морская сажень"}));
                    ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "ангстрем","нанометр","микрометр","мм","cм","дц","м","чейн","км","дюйм","фут","ярд","мил(mil)","морская миля","морская сажень"}));
                } else {//in English
                    FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "Angstrom (Å)","Nanometre (nm)","Micrometre (µm)","Millimetre (mm)","Centimetre (cm)","Decimetre (dm)","Metre (m)","Chain",
                        "Kilometre (km)","Inch (in)","Foot (ft)","Yard (yd)","Mil(mil)","Sea mile","Fathom"}));
                    ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "Angstrom (Å)","Nanometre (nm)","Micrometre (µm)","Millimetre (mm)","Centimetre (cm)","Decimetre (dm)","Metre (m)","Chain",
                        "Kilometre (km)","Inch (in)","Foot (ft)","Yard (yd)","Mil(mil)","Sea mile","Fathom"}));
                } break;
            case 1://square
                if (LangMenuItem) {//Russian
                    FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "нанометр²","микрометр²","мм²","см²","дц²","м²","Ар","Гектар","км²","Тауншип","миля²","Хомстед","Акр","Руд","Род²","Сквайр","Ярд²","Фут²","Дюйм²"}));
                    ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "нанометр²","микрометр²","мм²","см²","дц²","м²","Ар","Гектар","км²","Тауншип","миля²","Хомстед","Акр","Руд","Род²","Сквайр","Ярд²","Фут²","Дюйм²"}));
                } else {//in English
                    FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "nanometre²","micrometre²","millimetre²","centimetre² (cm²)","decimetre²","metre²","Are (a)","Hectare (ha)","kilometre²",
                        "Township","mile²","Homestead","Acre", "Rood", "rod²", "Square", "yard² (sy²)", "foot² (ft²)", "inch² (in²)"}));
                    ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "nanometre²","micrometre²","millimetre²","centimetre² (cm²)","decimetre²","metre²","Are (a)","Hectare (ha)","kilometre²",
                        "Township","mile²","Homestead","Acre", "Rood", "rod²", "Square", "yard² (sy²)", "foot² (ft²)", "inch² (in²)"}));
                } break;
    ...
    "ярд³","галлон","галлон (UK)","литр","дм³","кварта","кварта (UK)"}));
          ...
          FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
    "acre-foot","barrel","barrel (oil)","cm³","foot³","Mcf=10³cf","MMcf=10^6cf","Bcf=10^9cf","Tcf=10^12cf","inch³","m³",
    "yard³","gallon","gallon (UK)","liter","dm³","quart","quart (UK)"}));
             ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
    "acre-foot","barrel","barrel (oil)","cm³","foot³","Mcf=10³cf","MMcf=10^6cf","Bcf=10^9cf","Tcf=10^12cf","inch³","m³",
    "yard³","gallon","gallon (UK)","liter","dm³","quart","quart (UK)"}));
                    }
            ...
            case 3://density
                if (LangMenuItem) {//Russian
                    if (!UnitsListForm) { //Long List
                        FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "attogram/л", "centigram/л", "дециграмм/л", "dekagram/литр", "exagram/л", "femtogram/л", "гигаграмм/л", "г/см³", "г/м³", "г/мм³", "г/л", "гектограмм/л",
                        "кг/см³", "кг/м³", "кг/л", "мегаграмм/л", "микрограмм/л", "миллиграмм/см³", "миллиграмм/кубометр", "миллиграмм/мм³", "миллиграмм/л", "нанограм/л", "унция/фут³",
                    ...
                    if (!UnitsListForm) { //Long List 
                        ...
                        FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "gr/cm³","gr/liter","kg/m³","kg/liter","pound/foot³","pound/gallon","ppm","S.G.","gr/galUS"}));
                        ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                        "gr/cm³","gr/liter","kg/m³","kg/liter","pound/foot³","pound/gallon","ppm","S.G.","gr/galUS"}));
                    }
                }
            ...
            case 4://temperature
                if (LangMenuItem) {//Russian
                    FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"Цельсий", "Фаренгейт", "Кельвин", "Ранкина", "Реомюра"}));
                    ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"Цельсий", "Фаренгейт", "Кельвин", "Ранкина", "Реомюра"}));
                } else {//in English
                    FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"Celsius", "Fahrenheit", "Kelvin", "Rankine", "Réaumur"}));
                    ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"Celsius", "Fahrenheit", "Kelvin", "Rankine", "Réaumur"}));
                } break;
            case 5://pressure
                if (LangMenuItem) {//Russian
                    if (!UnitsListForm) {
                     ...
                     FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"атмосфера", "Бар", "дюйм ртуть (32°F)", "кг/см²", "килопаскаль", "МПа", "мм рт (0°C)", "Паскаль", "фунт/дюйм²"}));
                        ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"атмосфера", "Бар", "дюйм ртуть (32°F)", "кг/см²", "килопаскаль", "МПа", "мм рт (0°C)", "Паскаль", "фунт/дюйм²"}));
                    }
                } else {//in English
                    if (!UnitsListForm) {
                        FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{
                     ...
                     FromCmBox.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"atmosphere", "bar", "inch mercury (32°F)", "kg/cm²", "kilopascal", "megapascal", "mm mercury (0°C)", "pascal", "pound/inch²"}));
                        ToCmBx.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"atmosphere", "bar", "inch mercury (32°F)", "kg/cm²", "kilopascal", "megapascal", "mm mercury (0°C)", "pascal", "pound/inch²"}));
                    }
                } break;
            case 6://velocity
                if (LangMenuItem) {//Russian
    ...
    private javax.swing.JRadioButtonMenuItem FullMenu;
    ...
    private javax.swing.JRadioButtonMenuItem ShortMenu;
